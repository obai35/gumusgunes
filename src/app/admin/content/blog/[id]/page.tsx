...
export default function EditBlogPostPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/content/blog/${id}`)
      ...
      .catch(() => toast.error(ta('Failed to load post')))
  ...
  async function handleSubmit(publishStatus?: 'draft' | 'published') {
    ...
    } catch { toast.error(ta('Failed to update post')) }
  ...
  return (
    <div>
      <PageHeader title={ta('Edit Blog Post')} backHref="/admin/content/blog" />
      <div className="bg-white rounded-xl border border-gray-100 p-6 max-w-3xl">
        <div className="space-y-4">
          <div>
            ...
            <input value={title} onChange={e => setTitle(e.target.value)} placeholder={ta('Post title')} className="w-full px-3 py-2 border border-border rounded-lg text-sm" />
          </div>
          <div>
