...
export default function PurchasingTab() {
  ...
  const tabs: { key: SubTab; label: string; icon: any }[] = [
    { key: 'ap-aging', label: ta('AP Aging'), icon: Building2 },
    { key: 'bill-journal', label: ta('Bill Journal'), icon: Receipt },
    { key: 'inventory-valuation', label: ta('Inventory Valuation'), icon: Package },
  ]
...
function APAgingView() {
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/accounting/purchasing/ap-aging')
      ...
      .catch(() => toast.error(ta('Failed to load AP aging')))
      .finally(() => setLoading(false))
  }, [])

  if (loading) return <div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div>
  if (!aging) return <p className="text-sm text-muted-foreground py-4">{ta('No data')}</p>

  const bucketLabels: Record<string, string> = {
    current: ta('Current (Not Due)'),
    '1-30': ta('1–30 Days Overdue'),
    '31-60': ta('31–60 Days Overdue'),
    '61-90': ta('61–90 Days Overdue'),
    '90+': ta('90+ Days Overdue'),
  }
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-5 gap-3">
        {Object.entries(aging).map(([key, bucket]: [string, any]) => (
          <div key={key} className="p-4 bg-white rounded-xl border border-border">
            ...
            <p className="text-xs text-muted-foreground">{fmtNum(bucket.count)} {ta('bills')}</p>
          </div>
        ))}
      </div>
      <div className="p-4 bg-navy text-white rounded-xl flex justify-between items-center">
        <p className="font-medium">{ta('Total Accounts Payable')}</p>
      ...
      {Object.entries(aging).map(([key, bucket]: [string, any]) => bucket.bills.length > 0 && (
        <div key={key} className="bg-white rounded-xl border border-border overflow-hidden">
          ...
          >
            <span>{bucketLabels[key]} ({fmtNum(bucket.count)})</span>
            ...
            <div className="border-t border-border divide-y divide-border">
              {bucket.bills.map((bill: any) => (
                <div key={bill.id} className="px-4 py-2.5 flex items-center justify-between text-sm">
                  <div>
                    <p className="font-medium">{bill.supplier?.name || ta('Unknown Supplier')}</p>
                    <p className="text-xs text-muted-foreground">
                      #{bill.billNumber || bill.id.slice(0, 8)} · {ta('Due')} {fmtDate(new Date(bill.dueAt))}
                    </p>
                  </div>
...
function BillJournalView() {
  ...
  const loadBills = () => {
    ...
    fetch('/api/admin/accounting/bills?status=pending')
      ...
      .catch(() => toast.error(ta('Failed to load bills')))
      .finally(() => setLoading(false))
  }
  ...
  const postJournal = async (billId: string) => {
    ...
    try {
      ...
      toast.success(ta('Journal entry created'))
      loadBills()
    } catch (e: any) {
      toast.error(e.message || ta('Failed'))
    } finally {
      setPosting(null)
    }
  }
  ...
  const payBill = async (billId: string) => {
    ...
    try {
      ...
      const d = await r.json()
      if (!r.ok) throw new Error(d.error)
      toast.success(ta('Bill paid and journal entry created'))
      loadBills()
    } catch (e: any) {
      toast.error(e.message || ta('Failed'))
    } finally {
  ...
  return (
    <div className="space-y-3">
      {bills.length === 0 && <p className="text-sm text-muted-foreground py-4">{ta('No pending bills')}</p>}
      {bills.map(bill => (
        <div key={bill.id} className="bg-white rounded-xl border border-border p-4">
          <div className="flex items-center justify-between mb-2">
            <div>
              <p className="font-medium">{bill.supplier?.name || bill.supplierName || ta('Supplier')}</p>
              <p className="text-xs text-muted-foreground">
                #{bill.billNumber || bill.id.slice(0, 8)} · {ta('Due')} {fmtDate(new Date(bill.dueAt))}
              </p>
            </div>
          ...
          <div className="flex gap-2">
            ...
            >
              <DollarSign className="h-3 w-3" /> {ta('Record Payment')}
            </button>
          </div>
        </div>
      ))}
...
function InventoryValuationView() {
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/accounting/purchasing/inventory-valuation')
      ...
      .catch(() => toast.error(ta('Failed to load inventory valuation')))
      .finally(() => setLoading(false))
  }, [])

  if (loading) return <div className="flex justify-center py-8"><Loader2 className="h-6 w-6 animate-spin" /></div>
  if (!valuation) return <p className="text-sm text-muted-foreground py-4">{ta('No data')}</p>

  return (
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-4">
        <div className="p-4 bg-white rounded-xl border border-border">
          <p className="text-xs text-muted-foreground">{ta('Total Cost')}</p>
          <p className="text-xl font-bold text-navy">{formatCurrency(valuation.totalCost)}</p>
        </div>
        <div className="p-4 bg-white rounded-xl border border-border">
          <p className="text-xs text-muted-foreground">{ta('Total Retail Value')}</p>
        ...
        <div className="p-4 bg-white rounded-xl border border-border">
          ...
          <p className="text-xs text-muted-foreground">{fmtNum(valuation.totalUnits)} {ta('units')} {ta('across')} {fmtNum(valuation.itemCount)} {ta('products')}</p>
        </div>
      </div>

      <div className="bg-white rounded-xl border border-border overflow-hidden">
        <div className="px-4 py-3 border-b border-border font-medium text-sm">{ta('Inventory Detail')}</div>
          ...
          {valuation.items.map((p: any) => (
            <div key={p.id} className="px-4 py-2.5 flex items-center justify-between text-sm">
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">{p.sku || ta('No SKU')} · {fmtNum(p.stock)} {ta('units')}</p>
              </div>
              <div className="text-right">
                <p className="font-medium">{formatCurrency((p.costPrice ?? 0) * p.stock)}</p>
                <p className="text-xs text-muted-foreground">{formatCurrency(p.costPrice ?? 0)}/{ta('unit')}</p>
              </div>
            </div>
          ))}
        </div>
