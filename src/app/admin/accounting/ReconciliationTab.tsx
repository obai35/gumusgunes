...
export default function ReconciliationTab() {
  ...
  useEffect(() => {
    fetch('/api/admin/accounting/bank-accounts')
      ...
      .catch(() => { toast.error(ta('Failed to load bank accounts')); setLoading(false) })
  }, [])
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/accounting/bank-accounts/${selectedAccount}/transactions`)
      ...
      .catch(() => { toast.error(ta('Failed to load transactions')); setLoading(false) })
  }, [selectedAccount])
  ...
  async function handleImport() {
    ...
    input.onchange = async () => {
      ...
      if (res.ok) { toast.success(ta(`Imported ${transactions.length} transactions`)); window.location.reload() }
      else toast.error(ta('Import failed'))
    }
    input.click()
  }
  ...
  async function handleAutoMatch() {
    ...
    toast.success(ta(`Found ${d.suggestions?.length || 0} possible matches`))
  }
  ...
  async function handleMatch(txId: string, entryId: string) {
    ...
    if (res.ok) { toast.success(ta('Transaction matched')); setSelectedTx(null) }
    else toast.error(ta('Match failed'))
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <button onClick={handleImport} className="px-4 py-1.5 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors flex items-center gap-1.5">
          <Upload className="h-4 w-4" /> {ta('Import CSV')}
        </button>
        <button onClick={handleAutoMatch} className="px-4 py-1.5 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 transition-colors flex items-center gap-1.5">
          <Link2 className="h-4 w-4" /> {ta('Auto-match')}
        </button>
      </div>

      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-border p-4">
          <p className="text-xs text-muted-foreground mb-1">{ta('Total Transactions')}</p>
          <p className="text-xl font-bold text-navy">{transactions.length}</p>
        </div>
        <div className="bg-white rounded-xl border border-border p-4">
          <p className="text-xs text-muted-foreground mb-1">{ta('Cleared')}</p>
          <p className="text-xl font-bold text-green-600">{matched.length}</p>
        </div>
        <div className="bg-white rounded-xl border border-border p-4">
          <p className="text-xs text-muted-foreground mb-1">{ta('Outstanding')}</p>
      ...
      <div className="bg-white rounded-xl border border-border overflow-hidden">
        <div className="px-4 py-3 bg-gray-50 border-b border-border flex items-center justify-between">
          <h3 className="font-semibold text-navy">{ta('Bank Transactions')}</h3>
          <span className="text-xs text-muted-foreground">{fmtNum(unmatched.length)} {ta('unmatched')}</span>
        </div>
        <div className="overflow-x-auto max-h-96 overflow-y-auto">
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-white"><tr className="text-left text-muted-foreground border-b border-border"><th className="p-3 font-medium">{ta('Date')}</th><th className="p-3 font-medium">{ta('Description')}</th><th className="p-3 font-medium text-right">{ta('Debit')}</th><th className="p-3 font-medium text-right">{ta('Credit')}</th><th className="p-3 font-medium text-center">{ta('Status')}</th><th className="p-3 font-medium">{ta('Actions')}</th></tr></thead>
            <tbody>
              {transactions.map(tx => {
                ...
                return (
                  <tr key={tx.id} className={`border-b border-border/50 hover:bg-gray-50 ${tx.isReconciled ? 'bg-green-50/50' : suggestion ? 'bg-blue-50/50' : ''}`}>
                    <td className="p-3 text-navy">{fmtDate(tx.transactionDate)}</td>
                    <td className="p-3 text-navy">{tx.description}</td>
                    <td className="p-3 text-right text-red-600">{tx.debit > 0 ? fmtCurrency(tx.debit) : ''}</td>
                    <td className="p-3 text-right text-green-600">{tx.credit > 0 ? fmtCurrency(tx.credit) : ''}</td>
                    ...
                    <td className="p-3">
                      {!tx.isReconciled && (
                        <button onClick={() => { setSelectedTx(selectedTx?.id === tx.id ? null : tx); setMatchEntryId('') }} className="text-xs text-navy hover:text-gold">
                          {ta(selectedTx?.id === tx.id ? 'Cancel' : 'Match')}
                        </button>
                      )}
                    </td>
                  </tr>
      ...
      {selectedTx && (
        <div className="bg-blue-50 rounded-xl border border-blue-200 p-4">
          <h4 className="text-sm font-semibold text-navy mb-3">{ta(`Match Transaction: ${selectedTx.description}`)}</h4>
          <p className="text-xs text-muted-foreground mb-3">{ta(`Amount: ${fmtCurrency(selectedTx.debit + selectedTx.credit)}`)}</p>
          <p className="text-xs text-muted-foreground mb-2">{ta('Match with journal entry:')}</p>
          <input type="text" placeholder={ta('Enter journal entry ID...')} value={matchEntryId} onChange={e => setMatchEntryId(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm mb-3" />
          <button onClick={() => handleMatch(selectedTx.id, matchEntryId)} disabled={!matchEntryId} className="px-4 py-1.5 bg-navy text-silver rounded-lg text-sm font-medium hover:bg-navy/90 transition-colors disabled:opacity-50">{ta('Match')}</button>
        </div>
      )}
    </div>
  )
