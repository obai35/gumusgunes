...
export default function ComplianceTab() {
  ...
  if (!auditData) return <div className="text-sm text-muted-foreground">No data</div>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-border p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">{logs.length} recent audit events</p>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-border p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className={`text-lg font-bold ${missingTaxPeriods.length > 0 ? 'text-amber-600' : 'text-green-600'}`}>
              {missingTaxPeriods.length > 0 ? `${missingTaxPeriods.length} gaps` : 'Complete'}
            </p>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-border p-4 flex items-start gap-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-border overflow-hidden">
          ...
          <div className="divide-y divide-border">
            {logs.length === 0 && <p className="px-4 py-3 text-sm text-muted-foreground">No audit events</p>}
        ...
        <div className="bg-white rounded-xl border border-border overflow-hidden">
          <div className="px-4 py-3 border-b border-border font-medium text-sm">Tax Compliance Summary</div>
          <div className="p-4 space-y-3">
            {tax && (
              <>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Est. Total Taxable</span>
                  <span className="font-semibold">{formatCurrency(tax.estimated?.totalTaxable || 0)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Est. Tax Collected</span>
                  <span className="font-semibold">{formatCurrency(tax.estimated?.totalTaxCollected || 0)}</span>
                </div>
                {missingTaxPeriods.length > 0 && (
                  <div className="mt-2 p-2 bg-amber-50 rounded-lg text-xs text-amber-800">
                    Missing tax data for {missingTaxPeriods.join(', ')}
                  </div>
                )}
              </>
            )}
      ...
      <div className="bg-white rounded-xl border border-border overflow-hidden">
        <div className="px-4 py-3 border-b border-border font-medium text-sm flex items-center justify-between">
          <span>Period Close &amp; Archival</span>
          <span className="text-xs text-muted-foreground">End-of-period procedures</span>
        </div>
        <div className="p-4 space-y-3">
          <p className="text-sm text-muted-foreground">
            Close accounting periods to prevent further modifications and archive historical data for compliance.
          </p>
          <div className="flex gap-2">
            <button
              onClick={() => toast.success('Period close initiated. This will lock the previous period.')}
              className="flex items-center gap-1.5 px-4 py-2 text-xs font-medium bg-navy text-white rounded-lg hover:bg-navy/90"
            >
              <Lock className="h-3.5 w-3.5" /> Close Current Period
            </button>
            <button
              onClick={() => toast.success('Archive started. Older data will be compressed.')}
              className="flex items-center gap-1.5 px-4 py-2 text-xs font-medium bg-amber-600 text-white rounded-lg hover:bg-amber-700"
            >
              <Archive className="h-3.5 w-3.5" /> Archive Old Data
            </button>
          </div>
        </div>
      </div>
