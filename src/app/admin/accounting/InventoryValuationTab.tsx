...
export default function InventoryValuationTab() {
  ...
  function fetchValuation() {
    ...
    fetch(`/api/admin/accounting/inventory-valuation?date=${date}&method=${method}`)
      ...
      .catch(() => { toast.error(ta('Failed to load inventory valuation')); setLoading(false) })
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <select value={method} onChange={e => setMethod(e.target.value)} className="px-3 py-1.5 border border-border rounded-lg text-sm">
          <option value="weighted">{ta('Weighted Average')}</option>
          <option value="fifo">{ta('FIFO')}</option>
        </select>
        <button onClick={() => {
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-border p-4">
          <div className="flex items-center gap-2 mb-2"><Package className="h-4 w-4 text-blue-600" /><p className="text-xs text-muted-foreground">{ta('Total Products')}</p></div>
          <p className="text-2xl font-bold text-navy">{fmtNum(data.totalProducts)}</p>
        </div>
        <div className="bg-white rounded-xl border border-border p-4">
          <div className="flex items-center gap-2 mb-2"><DollarSign className="h-4 w-4 text-green-600" /><p className="text-xs text-muted-foreground">{ta('Total Value')}</p></div>
          <p className="text-2xl font-bold text-green-600">{fmtCurrency(data.totalValue)}</p>
        </div>
        <div className="bg-white rounded-xl border border-border p-4">
          <div className="flex items-center gap-2 mb-2"><TrendingUp className="h-4 w-4 text-purple-600" /><p className="text-xs text-muted-foreground">{ta('Total COGS')}</p></div>
          <p className="text-2xl font-bold text-purple-600">{fmtCurrency(data.totalCOGS)}</p>
        </div>
        <div className="bg-white rounded-xl border border-border p-4">
          <div className="flex items-center gap-2 mb-2"><TrendingUp className="h-4 w-4 text-emerald-600" /><p className="text-xs text-muted-foreground">{ta('Gross Margin')}</p></div>
          <p className={`text-2xl font-bold ${data.grossMargin >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{fmtNum(data.grossMargin, 1)}%</p>
        </div>
      </div>

      <div className="bg-white rounded-xl border border-border overflow-hidden">
        <div className="px-4 py-3 bg-gray-50 border-b border-border">
          <h3 className="font-semibold text-navy">{ta('Inventory Items')}</h3>
        </div>
        <div className="overflow-x-auto max-h-96 overflow-y-auto">
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-white"><tr className="text-left text-muted-foreground border-b border-border"><th className="p-3 font-medium">SKU</th><th className="p-3 font-medium">{ta('Name')}</th><th className="p-3 font-medium text-right">{ta('Qty')}</th><th className="p-3 font-medium text-right">{ta('Unit Cost')}</th><th className="p-3 font-medium text-right">{ta('Total Value')}</th></tr></thead>
            <tbody>
              {data.items.map((item: any) => (
                <tr key={item.id} className="border-b border-border/50 hover:bg-gray-50">
                  ...
                  <td className="p-3 text-right">{fmtNum(item.quantity)}</td>
                  <td className="p-3 text-right">{fmtCurrency(item.unitCost)}</td>
                  <td className="p-3 text-right font-semibold">{fmtCurrency(item.totalValue)}</td>
                </tr>
              ))}
            </tbody>
          </table>
