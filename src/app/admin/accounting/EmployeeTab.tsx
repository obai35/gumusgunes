...
export default function EmployeeTab() {
  ...
  function fetchEmployees() {
    ...
    fetch('/api/admin/accounting/employees')
      ...
      .catch(() => { toast.error(ta('Failed to load employees')); setLoading(false) })
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast.success(ta(editing ? 'Employee updated' : 'Employee created'))
      setShowModal(false)
      fetchEmployees()
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-navy">{ta('Employees')}</h2>
        <button onClick={openCreate} className="px-4 py-2 bg-navy text-silver rounded-lg text-sm font-medium hover:bg-navy/90 transition-colors flex items-center gap-1.5">
          <Plus className="h-4 w-4" /> {ta('Add Employee')}
        </button>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-gray-50 text-left text-muted-foreground">
              <th className="p-3 font-medium">{ta('Name')}</th>
              <th className="p-3 font-medium">{ta('Position')}</th>
              <th className="p-3 font-medium text-right">{ta('Salary')}</th>
              <th className="p-3 font-medium">{ta('Bank')}</th>
              <th className="p-3 font-medium">{ta('Actions')}</th>
            </tr>
          </thead>
          <tbody>
            {employees.length === 0 && <tr><td colSpan={5} className="p-6 text-center text-muted-foreground">{ta('No employees yet')}</td></tr>}
            {employees.map(emp => (
              <tr key={emp.id} className="border-b border-border/50 hover:bg-gray-50">
                ...
                <td className="p-3 text-right font-medium text-navy">{fmtCurrency(emp.salary)}</td>
                ...
                <td className="p-3">
                  <button onClick={() => openEdit(emp)} className="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded hover:bg-blue-200 transition-colors">
                    <Pencil className="h-3 w-3 inline mr-1" /> {ta('Edit')}
                  </button>
                </td>
              </tr>
            ))}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-xl max-w-lg w-full p-6 space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center">
              <h3 className="font-semibold text-navy">{ta(editing ? 'Edit Employee' : 'Add Employee')}</h3>
              <button onClick={() => setShowModal(false)}><X className="h-5 w-5 text-muted-foreground hover:text-navy" /></button>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                <label className="text-xs text-muted-foreground mb-1 block">{ta('Name *')}</label>
              ...
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">{ta('Email')}</label>
              ...
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">{ta('Phone')}</label>
              ...
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">{ta('Position')}</label>
              ...
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">{ta('Salary (EGP)')}</label>
              ...
              <div className="col-span-2 border-t border-border pt-3">
                <p className="text-xs font-medium text-navy mb-2">{ta('Bank Details (for payroll)')}</p>
              </div>
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">{ta('Bank Name')}</label>
              ...
              <div>
                <label className="text-xs text-muted-foreground mb-1 block">{ta('Account Number')}</label>
            ...
            <div className="flex gap-2 justify-end pt-2">
              ...
              <button onClick={handleSave} className="px-4 py-2 bg-navy text-silver rounded-lg text-sm font-medium hover:bg-navy/90 transition-colors">{ta('Save')}</button>
            </div>
          </div>
        </div>
      )}
