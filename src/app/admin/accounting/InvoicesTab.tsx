...
export default function InvoicesTab() {
  ...
  function fetchInvoices() {
    ...
    fetch(`/api/admin/accounting/invoices?${params}`)
      ...
      .catch(() => { toast.error(ta('Failed to load invoices')); setLoading(false) })
  }
  ...
  async function updateStatus(id: string, status: string) {
    ...
    if (res.ok) { toast.success(ta(`Invoice ${status}`)); fetchInvoices() }
    else toast.error(ta('Failed to update'))
  }

  async function deleteInvoice(id: string) {
    if (!confirm(ta('Delete this invoice?'))) return
    const res = await fetch(`/api/admin/accounting/invoices/${id}`, { method: 'DELETE' })
    if (res.ok) { toast.success(ta('Invoice deleted')); fetchInvoices() }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 max-w-xs">
          ...
          <input type="text" placeholder={ta('Search invoices...')} value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') fetchInvoices() }} className="w-full pl-9 pr-3 py-2 border border-border rounded-lg text-sm" />
        </div>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="px-3 py-2 border border-border rounded-lg text-sm">
          ...
          <option value="sent">{ta('Sent')}</option>
        ...
        <button onClick={() => setShowCreateModal(true)} className="px-4 py-2 bg-navy text-silver rounded-lg text-sm font-medium hover:bg-navy/90 transition-colors flex items-center gap-1.5">
          <Plus className="h-4 w-4" /> {ta('Create Invoice')}
        </button>
        <button onClick={() => {
          ...
          <tbody>
            {invoices.map(inv => (
              <tr key={inv.id} className="border-b border-border/50 hover:bg-gray-50">
                ...
                <td className="p-3 text-right font-semibold">{fmtCurrency(inv.total)}</td>
                ...
                <td className="p-3 text-muted-foreground text-xs">{fmtDate(inv.issuedAt)}</td>
                <td className="p-3 text-muted-foreground text-xs">{inv.dueAt ? fmtDate(inv.dueAt) : '-'}</td>
                <td className="p-3">
                  <div className="flex items-center justify-center gap-1">
                    {inv.status === 'draft' && <button onClick={() => updateStatus(inv.id, 'sent')} className="p-1 hover:bg-blue-50 rounded" title={ta('Send')}><Send className="h-3.5 w-3.5 text-blue-600" /></button>}
                    {inv.status === 'sent' && <button onClick={() => updateStatus(inv.id, 'paid')} className="p-1 hover:bg-green-50 rounded" title={ta('Mark Paid')}><CheckCircle className="h-3.5 w-3.5 text-green-600" /></button>}
                    {inv.status !== 'paid' && inv.status !== 'cancelled' && <button onClick={() => updateStatus(inv.id, 'cancelled')} className="p-1 hover:bg-red-50 rounded" title={ta('Cancel')}><XCircle className="h-3.5 w-3.5 text-red-600" /></button>}
                    {inv.status === 'draft' && <button onClick={() => deleteInvoice(inv.id)} className="p-1 hover:bg-red-50 rounded" title={ta('Delete')}><XCircle className="h-3.5 w-3.5 text-slate-400" /></button>}
                  </div>
                </td>
              </tr>
            ))}
            {invoices.length === 0 && <tr><td colSpan={7} className="p-8 text-center text-muted-foreground">{ta('No invoices found')}</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50" onClick={() => setShowCreateModal(false)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-lg mx-4 shadow-xl" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-navy mb-4">{ta('Create Invoice')}</h3>
            <p className="text-sm text-muted-foreground">{ta('Invoice creation form: customer details, line items, totals.')}</p>
            <p className="text-xs text-muted-foreground mt-2">{ta('Option: Create from Order (enter order ID to auto-fill).')}</p>
            <div className="flex gap-2 mt-4">
              ...
              <button onClick={async () => {
                ...
                if (res.ok) { toast.success(ta('Invoice created')); setShowCreateModal(false); fetchInvoices() }
                else toast.error(ta('Failed to create'))
              }} className="px-4 py-2 bg-navy text-silver rounded-lg text-sm font-medium">{ta('Create Draft')}</button>
            </div>
          </div>
        </div>
      )}
