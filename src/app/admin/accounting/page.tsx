...
export default function AccountingPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/accounting/overview?${params}`)
      ...
      .catch(() => { toast.error(ta('Failed to load data')); setOverviewLoading(false) })
  }, [period, customStart, customEnd, refreshKey, tab])

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <h1 className="text-2xl font-display font-semibold text-navy">{ta('Accounting')}</h1>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        <StatCard label={ta('Total Revenue')} value={formatCurrency(data.totalRevenue)} icon={DollarSign} color="text-green-600" bg="bg-green-50" compareKey="compareRevenue" onClick={() => setDrillDown({ type: 'Total Revenue', data: { revenue: data.totalRevenue, period } })} />
        <StatCard label={ta('Net Revenue')} value={formatCurrency(data.netRevenue)} icon={TrendingUp} color="text-emerald-600" bg="bg-emerald-50" compareKey="compareNetRevenue" onClick={() => setDrillDown({ type: 'Net Revenue', data: { netRevenue: data.netRevenue, totalRevenue: data.totalRevenue, returns: data.totalReturns, expenses: data.totalExpenses, period } })} />
        <StatCard label={ta('Total Orders')} value={data.totalOrders} icon={Receipt} color="text-navy" bg="bg-blue-50" compareKey="compareTotalOrders" onClick={() => setDrillDown({ type: 'Orders', data: { totalOrders: data.totalOrders, pendingOrders: data.pendingOrders, period } })} />
        <StatCard label={ta('Avg Order')} value={formatCurrency(data.avgOrderValue)} icon={Wallet} color="text-purple-600" bg="bg-purple-50" onClick={() => setDrillDown({ type: 'Avg Order Value', data: { avgOrderValue: data.avgOrderValue, totalRevenue: data.totalRevenue, totalOrders: data.totalOrders, period } })} />
        <StatCard label={ta('Returns')} value={formatCurrency(data.totalReturns)} icon={TrendingDown} color="text-red-600" bg="bg-red-50" onClick={() => setDrillDown({ type: 'Returns', data: { totalReturns: data.totalReturns, period } })} />
        <StatCard label={ta('Expenses')} value={formatCurrency(data.totalExpenses)} icon={Banknote} color="text-orange-600" bg="bg-orange-50" />
      </div>

      {data.budgetComparison && data.budgetComparison.budgeted > 0 && (
        <div className="bg-white rounded-xl border border-border p-4">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs text-muted-foreground uppercase tracking-wide">{ta('Budget vs Actual')}</p>
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1">
              <div className="flex justify-between text-sm mb-1">
                <span className="text-muted-foreground">{ta('Actual')}</span>
      ...
      {data.totalRevenue > 0 && data.totalReturns !== undefined && (
        <div className="bg-white rounded-xl border border-border p-5">
          <h3 className="text-sm font-semibold text-navy mb-3">{ta('COGS Reconciliation')}</h3>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <p className="text-xs text-muted-foreground">{ta('COGS')} {data.totalCOGS === undefined && ta('(Est.)')}</p>
              <p className="text-lg font-bold text-amber-600">{formatCurrency(data.totalCOGS ?? data.totalRevenue * 0.6)}</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">{ta('Gross Margin')}</p>
      ...
      {ratios && (
        <div className="bg-white rounded-xl border border-border p-5">
          <h3 className="text-sm font-semibold text-navy mb-4">{ta('Financial Ratios')}</h3>
            ...
            {Object.entries(ratios.profitability || {}).map(([key, r]: [string, any]) => (
              <div key={key} className="p-3 rounded-lg bg-gray-50">
                ...
                {r.benchmark !== undefined && (
                  <p className="text-xs text-muted-foreground mt-0.5">{ta('Benchmark')}: {fmtNum(r.benchmark * 100)}%</p>
                )}
              </div>
            ))}
      ...
      <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="flex items-center gap-4">
            <ResponsiveContainer width={140} height={140}>
              <PieChart>
                <Pie
                  data={[
                    { name: ta('Cash'), value: data.paymentBreakdown?.cash || 0 },
                    { name: ta('Card'), value: data.paymentBreakdown?.card || 0 },
                    { name: ta('Split'), value: data.paymentBreakdown?.split || 0 },
                    { name: ta('Bank Transfer'), value: data.paymentBreakdown?.bank_transfer || 0 },
                    { name: ta('InstaPay'), value: data.paymentBreakdown?.instapay || 0 },
                    { name: ta('Wallet'), value: data.paymentBreakdown?.wallet || 0 },
                  ].filter(d => d.value > 0)}
        ...
        <div className="bg-white rounded-xl border border-border p-5">
          ...
          {Object.entries(data.branchRevenue || {}).length === 0 ? (
            <p className="text-sm text-muted-foreground">{ta('No data for this period')}</p>
          ) : (
      ...
      <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="space-y-2 flex-1">
              <div className="flex items-center justify-between text-xs">
                <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-emerald-500" />{ta('Cash')}</span>
                <span className="font-medium text-navy">{formatCurrency(cp1000)}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-blue-500" />{ta('Bank')}</span>
                <span className="font-medium text-navy">{formatCurrency(cp1100)}</span>
              </div>
              <div className="pt-1 border-t border-border">
                <div className="flex items-center justify-between text-xs font-semibold">
                  <span>{ta('Total Liquid')}</span>
        ...
        <div className="bg-white rounded-xl border border-border p-5">
          <h3 className="text-sm font-semibold text-navy mb-4 flex items-center gap-2">
            ...
            {ta('Expense Breakdown')}
          </h3>
          {(data.expenseBreakdown || []).length === 0 ? (
            <p className="text-sm text-muted-foreground">{ta('No expenses this period')}</p>
          ) : (
              ...
              {data.expenseBreakdown.slice(0, 8).map((exp: any) => {
                ...
                return (
                  <div key={exp.code}>
                    ...
                    <span className="text-[10px] text-muted-foreground">{fmtNum(pct, 1)}%</span>
                  </div>
                )
              })}
              {(data.expenseBreakdown?.length || 0) > 8 && (
                <p className="text-xs text-muted-foreground text-center pt-1">+{fmtNum(data.expenseBreakdown.length - 8)} {ta('more')}</p>
              )}
            </div>
          )}
        </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-border p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">{ta('Pending Orders')}</p>
          <p className="text-2xl font-bold text-amber-600">{fmtNum(data.pendingOrders)}</p>
        </div>
        <div className="bg-white rounded-xl border border-border p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">{ta('Unreconciled Payments')}</p>
          <p className="text-2xl font-bold text-red-600">{fmtNum(data.unreconciledOrders)}</p>
        </div>
        <div className="bg-white rounded-xl border border-border p-4">
          <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">{ta('Open Shifts')}</p>
          <p className={`text-2xl font-bold ${data.openShifts > 0 ? 'text-amber-600' : 'text-green-600'}`}>{fmtNum(data.openShifts)}</p>
      ...
      <div className="bg-gradient-to-r from-navy to-navy/90 rounded-xl p-5 text-white">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <p className="text-xs opacity-60">{ta(PERIOD_LABELS[period])}</p>
      ...
      {drillDown && (
        <div className="bg-gray-50 rounded-xl border border-border p-4">
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-sm font-semibold text-navy">{ta('Drill-down')}: {drillDown.type}</h4>
            <button onClick={() => setDrillDown(null)} className="text-xs text-muted-foreground hover:text-navy">{ta('Close')}</button>
          </div>
...
function OrdersTab() {
  const { ta, fmtNum, fmtDate, fmtDateTime } = useAdminTranslate()
  ...
  async function handleFulfill(id: string) {
    try {
      ...
      if (res.ok) { toast.success(ta('Order fulfilled')); fetchOrders() }
      else toast.error(ta('Failed to fulfill'))
    } catch { toast.error(ta('Failed to fulfill')) }
  }

  async function handleReconcile(id: string) {
    try {
      const res = await fetch(`/api/admin/accounting/orders/${id}/reconcile`, { method: 'POST' })
      if (res.ok) { toast.success(ta('Payment reconciled')); fetchOrders() }
      else toast.error(ta('Failed to reconcile'))
    } catch { toast.error(ta('Failed to reconcile')) }
  }
    ...
    return (
      <div>
        <button onClick={() => setSelectedOrder(null)} className="text-sm text-navy hover:text-gold mb-4 flex items-center gap-1">
          <X className="h-4 w-4" /> {ta('Back to orders')}
        </button>
        <div className="bg-white rounded-xl border border-border p-6 space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div><p className="text-muted-foreground">{ta('Order #')}</p><p className="font-medium text-navy">{selectedOrder.receiptNumber || selectedOrder.orderNumber}</p></div>
            <div><p className="text-muted-foreground">{ta('Customer')}</p><p className="font-medium text-navy">{selectedOrder.fullName}</p></div>
            <div><p className="text-muted-foreground">{ta('Branch')}</p><p className="font-medium text-navy">{selectedOrder.shift?.branch?.name || '-'}</p></div>
            <div><p className="text-muted-foreground">{ta('Total')}</p><p className="font-bold text-navy">{formatCurrency(selectedOrder.totalAmount)}</p></div>
            <div><p className="text-muted-foreground">{ta('Status')}</p><span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${statusColors[selectedOrder.status] || ''}`}>{selectedOrder.status}</span></div>
            <div><p className="text-muted-foreground">{ta('Payment')}</p><p className="font-medium text-navy">{paymentLabels[selectedOrder.paymentMethod] || selectedOrder.paymentMethod}</p></div>
            <div><p className="text-muted-foreground">{ta('Payment Status')}</p><p className="font-medium text-navy">{selectedOrder.paymentStatus}</p></div>
            <div><p className="text-muted-foreground">{ta('Date')}</p><p className="font-medium text-navy">{fmtDateTime(selectedOrder.createdAt)}</p></div>
            {selectedOrder.fulfilledAt && <div><p className="text-muted-foreground">{ta('Fulfilled')}</p><p className="font-medium text-green-600">{fmtDateTime(selectedOrder.fulfilledAt)}</p></div>}
            {selectedOrder.reconciledAt && <div><p className="text-muted-foreground">{ta('Reconciled')}</p><p className="font-medium text-green-600">{fmtDateTime(selectedOrder.reconciledAt)}</p></div>}
          </div>
          {selectedOrder.items?.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold text-navy mb-2">{ta('Items')}</h3>
              <table className="w-full text-sm">
                <thead><tr className="border-b border-border text-left text-muted-foreground"><th className="pb-2 font-medium">{ta('Product')}</th><th className="pb-2 font-medium text-right">{ta('Qty')}</th><th className="pb-2 font-medium text-right">{ta('Price')}</th><th className="pb-2 font-medium text-right">{ta('Total')}</th></tr></thead>
                <tbody>
                  {selectedOrder.items.map((item: any, i: number) => (
                    <tr key={item.id || i} className="border-b border-border/50">
                      ...
                      <td className="py-2 text-right text-muted-foreground">{fmtNum(item.quantity)}</td>
          ...
          <div className="flex gap-2 pt-2">
            {selectedOrder.status !== 'delivered' && (
              <button onClick={() => { handleFulfill(selectedOrder.id); setSelectedOrder(null) }} className="px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700 transition-colors flex items-center gap-1.5">
                <CheckCircle className="h-4 w-4" /> {ta('Mark Delivered')}
              </button>
            )}
            {!selectedOrder.reconciledAt && selectedOrder.paymentStatus === 'paid' && (
              <button onClick={() => { handleReconcile(selectedOrder.id); setSelectedOrder(null) }} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors flex items-center gap-1.5">
                <DollarSign className="h-4 w-4" /> {ta('Reconcile Payment')}
              </button>
            )}
          </div>
        </div>
      ...
      <div className="flex gap-3 mb-4 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            placeholder={ta('Search by order#, receipt, name...')}
        ...
        <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1) }} className="px-3 py-2 border border-border rounded-lg text-sm">
          ...
          <option value="processing">{ta('Processing')}</option>
          <option value="shipped">{ta('Shipped')}</option>
          <option value="delivered">{ta('Delivered')}</option>
        ...
        <button onClick={() => {
          ...
          <Download className="h-4 w-4" /> {ta('Export Excel')}
        </button>
        <button onClick={fetchOrders} className="px-4 py-2 bg-navy text-silver rounded-lg text-sm font-medium hover:bg-navy/90 transition-colors">
          <Filter className="h-4 w-4 inline mr-1" /> {ta('Filter')}
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-border overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-gray-50 text-left text-muted-foreground">
                <th className="p-3 font-medium">{ta('Receipt #')}</th>
                <th className="p-3 font-medium">{ta('Branch')}</th>
                <th className="p-3 font-medium">{ta('Customer')}</th>
                <th className="p-3 font-medium text-right">{ta('Total')}</th>
                <th className="p-3 font-medium">{ta('Status')}</th>
                <th className="p-3 font-medium">{ta('Payment')}</th>
                <th className="p-3 font-medium">{ta('Reconciled')}</th>
                <th className="p-3 font-medium">{ta('Date')}</th>
                <th className="p-3 font-medium">{ta('Actions')}</th>
              </tr>
            </thead>
            <tbody>
              {orders.length === 0 && <tr><td colSpan={9} className="p-6 text-center text-muted-foreground">{ta('No orders found')}</td></tr>}
              {orders.map((order) => (
                <tr key={order.id} className="border-b border-border/50 hover:bg-gray-50 cursor-pointer" onClick={() => setSelectedOrder(order)}>
                  ...
                  <td className="p-3">{order.reconciledAt ? <span className="text-green-600 text-xs font-medium">{ta('Yes')}</span> : <span className="text-amber-600 text-xs font-medium">{ta('No')}</span>}</td>
                  <td className="p-3 text-muted-foreground text-xs">{fmtDate(order.createdAt)}</td>
                  <td className="p-3">
                    <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                      {order.status !== 'delivered' && <button onClick={() => handleFulfill(order.id)} className="px-2 py-1 text-xs bg-green-100 text-green-700 rounded hover:bg-green-200 transition-colors">{ta('Fulfill')}</button>}
                      {!order.reconciledAt && order.paymentStatus === 'paid' && <button onClick={() => handleReconcile(order.id)} className="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded hover:bg-blue-200 transition-colors">{ta('Reconcile')}</button>}
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <div className="p-3 flex items-center justify-between text-sm text-muted-foreground border-t border-border">
            <span>{fmtNum(total)} {ta('total orders')}</span>
...
function BranchesTab() {
  ...
  useEffect(() => {
    fetch(`/api/admin/accounting/branches?period=${period}`)
      .then((r) => { if (!r.ok) throw new Error(); return r.json() })
      .then(setData)
      .catch(() => toast.error(ta('Failed to load branch data')))
  }, [period])
          ...
          {(Array.isArray(data.branches) ? data.branches : []).map((branch: any) => (
            <div key={branch.id} className="bg-white rounded-xl border border-border p-5 hover:shadow-md transition-shadow">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-muted-foreground">{ta('Revenue')}</span><span className="font-bold text-navy">{formatCurrency(branch.totalRevenue)}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">{ta('Orders')}</span><span className="font-medium text-navy">{fmtNum(branch.orderCount)}</span></div>
                <div className="border-t border-border pt-2 mt-2 space-y-1">
                  <div className="flex justify-between text-xs"><span className="text-green-600">{ta('Cash')}</span><span className="font-medium text-navy">{formatCurrency(branch.cashTotal)}</span></div>
                  <div className="flex justify-between text-xs"><span className="text-blue-600">{ta('Card')}</span><span className="font-medium text-navy">{formatCurrency(branch.cardTotal)}</span></div>
                  <div className="flex justify-between text-xs"><span className="text-purple-600">{ta('Other')}</span><span className="font-medium text-navy">{formatCurrency(branch.otherTotal)}</span></div>
                </div>
              </div>
            </div>
          ))}
          {(!Array.isArray(data.branches) || data.branches.length === 0) && <div className="col-span-full text-center text-muted-foreground text-sm py-8">{ta('No data for this period')}</div>}
        </div>
      )}
    </div>
  )
...
function ExpensesTab({ refreshKey }: { refreshKey: number }) {
  ...
  function fetchExpenses() {
    ...
    fetch(`/api/admin/accounting/expenses?${params}`)
      ...
      .catch(() => { toast.error(ta('Failed to load expenses')); setLoading(false) })
  }
  ...
  async function handleDeleteExpense(id: string) {
    try {
      ...
      if (res.ok) { toast.success(ta('Expense deleted')); fetchExpenses(); setDeleteConfirm(null) }
    ...
    <div>
      <div className="flex gap-2 mb-4 flex-wrap items-center">
        ...
        <button onClick={() => setShowModal(true)} className="px-4 py-1.5 bg-navy text-silver rounded-lg text-sm font-medium hover:bg-navy/90 transition-colors flex items-center gap-1.5">
          <Plus className="h-4 w-4" /> {ta('Add Expense')}
        </button>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-white rounded-xl border border-border p-4">
              <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">{ta('Total Expenses')}</p>
              <p className="text-2xl font-bold text-orange-600">{formatCurrency(data.totalExpenses)}</p>
            </div>
            <div className="bg-white rounded-xl border border-border p-4">
              <p className="text-xs text-muted-foreground uppercase tracking-wide mb-1">{ta('Count')}</p>
              <p className="text-2xl font-bold text-navy">{fmtNum(data.count)}</p>
            </div>
          </div>

          <div className="grid md:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-xl border border-border p-5">
              <h3 className="text-sm font-semibold text-navy mb-4">{ta('Payment Method Breakdown')}</h3>
              <div className="space-y-3">
                {Object.entries(data.byMethod || {}).length === 0 ? (
                  <p className="text-sm text-muted-foreground">{ta('No expenses')}</p>
                ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-gray-50 text-left text-muted-foreground">
                  ...
                  <th className="p-3 font-medium">{ta('Payment Method')}</th>
                  <th className="p-3 font-medium">{ta('Branch')}</th>
                  <th className="p-3 font-medium">{ta('Supplier')}</th>
                  <th className="p-3 font-medium">{ta('Invoice #')}</th>
                  <th className="p-3 font-medium">{ta('Actions')}</th>
                </tr>
              </thead>
              <tbody>
                {(!Array.isArray(data.expenses) || data.expenses.length === 0) && <tr><td colSpan={8} className="p-6 text-center text-muted-foreground">{ta('No expenses found')}</td></tr>}
                {Array.isArray(data.expenses) && data.expenses.map((exp: any) => (
                  <tr key={exp.id} className="border-b border-border/50 hover:bg-gray-50">
                    <td className="p-3 text-muted-foreground text-xs">{fmtDate(exp.createdAt)}</td>
                    ...
                    <td className="p-3">
                      {deleteConfirm === exp.id ? (
                        <div className="flex gap-1">
                          <button onClick={() => handleDeleteExpense(exp.id)} className="px-2 py-1 text-xs bg-red-600 text-white rounded hover:bg-red-700 transition-colors">{ta('Confirm')}</button>
                          <button onClick={() => setDeleteConfirm(null)} className="px-2 py-1 text-xs bg-gray-200 text-gray-700 rounded hover:bg-gray-300 transition-colors">{ta('Cancel')}</button>
                        </div>
                      ) : (
...
function AddExpenseModal({ onClose, onSaved, branches }: { onClose: () => void; onSaved: () => void; branches: any[] }) {
  ...
  async function handleSave() {
    if (!amount || !description || !paymentMethod) {
      toast.error(ta('Amount, description, and payment method required'))
      return
    }
    setSaving(true)
    try {
      ...
      if (res.ok) { toast.success(ta('Expense added')); onSaved() }
      else { const d = await res.json(); toast.error(d.error || ta('Failed')) }
    } catch { toast.error(ta('Failed to add expense')) }
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-navy">{ta('Add Expense')}</h2>
          <button onClick={onClose} className="p-1 hover:bg-gray-100 rounded"><X className="h-5 w-5 text-muted-foreground" /></button>
        </div>
        <div className="space-y-3">
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">{ta('Amount *')}</label>
          ...
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">{ta('Description *')}</label>
            <input type="text" value={description} onChange={e => setDescription(e.target.value)} placeholder={ta('What is this expense for?')} className="w-full px-3 py-2 border border-border rounded-lg text-sm" />
          </div>
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">{ta('Payment Method *')}</label>
            <select value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)} className="w-full px-3 py-2 border border-border rounded-lg text-sm">
              ...
              <option value="card">{ta('Card')}</option>
              <option value="bank_transfer">{ta('Bank Transfer')}</option>
              <option value="instapay">{ta('InstaPay')}</option>
              <option value="wallet">{ta('Wallet')}</option>
            </select>
          </div>
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">{ta('Branch')}</label>
          ...
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">{ta('Supplier')}</label>
          ...
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">{ta('Invoice Number')}</label>
            <input type="text" value={invoiceNumber} onChange={e => setInvoiceNumber(e.target.value)} placeholder={ta('Optional')} className="w-full px-3 py-2 border border-border rounded-lg text-sm" />
          </div>
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1">{ta('Notes')}</label>
            <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder={ta('Optional notes')} rows={2} className="w-full px-3 py-2 border border-border rounded-lg text-sm" />
          </div>
        </div>
        <div className="flex gap-2 pt-2">
          <button onClick={onClose} className="flex-1 px-4 py-2 border border-border rounded-lg text-sm font-medium text-muted-foreground hover:text-navy transition-colors">{ta('Cancel')}</button>
          <button onClick={handleSave} disabled={saving} className="flex-1 px-4 py-2 bg-navy text-silver rounded-lg text-sm font-medium hover:bg-navy/90 transition-colors disabled:opacity-50">
            {saving ? ta('Saving...') : ta('Save Expense')}
          </button>
        </div>
      </motion.div>
    </motion.div>
