...
export default function PayrollTab() {
  ...
  function fetchRuns() {
    ...
    fetch('/api/admin/accounting/payroll-runs')
      ...
      .catch(() => { toast.error(ta('Failed to load payroll runs')); setLoading(false) })
  }

  useEffect(() => { fetchRuns() }, [])

  async function handleCreate() {
    if (!form.periodStart || !form.periodEnd) { toast.error(ta('Select period dates')); return }
    try {
      ...
      toast.success(ta('Payroll run created'))
      setShowModal(false)
      fetchRuns()
    } catch { toast.error(ta('Failed to create payroll run')) }
  }
  ...
  async function handleAction(id: string, action: string) {
    try {
      ...
      if (!res.ok) throw new Error()
      toast.success(ta(`Payroll ${action}d`))
      fetchRuns()
    } catch { toast.error(ta(`Failed to ${action}`)) }
  }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-navy">{ta('Payroll Runs')}</h2>
          ...
          {runs.map(run => (
            <div key={run.id} className="bg-white rounded-xl border border-border p-5">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {ta(`${run.items?.length || 0} employees · Created ${fmtDate(run.createdAt)}`)}
                    {run.processedBy && ta(` · by ${run.processedBy.name}`)}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-3 gap-4 mb-3">
                <div>
                  <p className="text-xs text-muted-foreground">{ta('Gross Salaries')}</p>
                  <p className="text-lg font-bold text-navy">{fmtCurrency(run.totalSalaries)}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">{ta('Deductions')}</p>
                  <p className="text-lg font-bold text-red-600">-{fmtCurrency(run.totalDeductions)}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">{ta('Net Pay')}</p>
                  <p className="text-lg font-bold text-green-600">{fmtCurrency(run.totalNet)}</p>
                </div>
              </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-left text-muted-foreground text-xs">
                        <th className="pb-2 font-medium">{ta('Employee')}</th>
                        <th className="pb-2 font-medium text-right">{ta('Salary')}</th>
                        <th className="pb-2 font-medium text-right">{ta('Bonus')}</th>
                        <th className="pb-2 font-medium text-right">{ta('Deductions')}</th>
                        <th className="pb-2 font-medium text-right">{ta('Net')}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {run.items.map((item: any) => (
                        <tr key={item.id} className="border-t border-border/50">
                          ...
                          <td className="py-1.5 text-right">{fmtCurrency(item.salary)}</td>
                          <td className="py-1.5 text-right text-green-600">{item.bonus > 0 ? fmtCurrency(item.bonus) : '-'}</td>
                          <td className="py-1.5 text-right text-red-600">{item.deductions > 0 ? fmtCurrency(item.deductions) : '-'}</td>
                          <td className="py-1.5 text-right font-medium text-navy">{fmtCurrency(item.netPay)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="flex gap-2 mt-3 pt-3 border-t border-border">
                ...
                {run.status === 'approved' && (
                  <button onClick={() => handleAction(run.id, 'pay')} className="px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs font-medium hover:bg-green-700 transition-colors flex items-center gap-1">
                    <DollarSign className="h-3.5 w-3.5" /> {ta('Mark Paid')}
                  </button>
                )}
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-xl max-w-md w-full p-6 space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center">
              <h3 className="font-semibold text-navy">{ta('New Payroll Run')}</h3>
              <button onClick={() => setShowModal(false)}><X className="h-5 w-5 text-muted-foreground hover:text-navy" /></button>
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{ta('Period Start')}</label>
            ...
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{ta('Period End')}</label>
            ...
            <div className="flex gap-2 justify-end pt-2">
              ...
              <button onClick={handleCreate} className="px-4 py-2 bg-navy text-silver rounded-lg text-sm font-medium hover:bg-navy/90 transition-colors">{ta('Create')}</button>
            </div>
          </div>
        </div>
      )}
