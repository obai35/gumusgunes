        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="rounded-lg border p-4 space-y-3">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} className="rounded border px-3 py-2 text-sm">
              {formulaTypes.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
            </select>
            <input type="number" step="any" placeholder={form.type === 'margin' ? 'Target margin % (e.g. 40)' : form.type === 'markup_multiplier' ? 'Multiplier (e.g. 2.5)' : 'Fixed amount'} value={form.value} onChange={e => setForm({ ...form, value: e.target.value })} className="rounded border px-3 py-2 text-sm" />
            <input placeholder="Description" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className="rounded border px-3 py-2 text-sm col-span-2" />
          ...
          return (
            <motion.div key={f.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-lg border bg-card p-4">
              <div className="flex items-start justify-between mb-1">
                ...
                {f.isDefault && <span className="text-[10px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded font-medium">Default</span>}
              </div>
              ...
              <div className="mt-3 pt-3 border-t border-border flex items-center justify-between">
                <span className="text-lg font-bold font-mono">
                  {f.type === 'margin' ? `${f.value}%` : f.type === 'markup_multiplier' ? `×${f.value}` : `${f.value} ${f.currency}`}
                </span>
                <div className="flex gap-2">
                  <button onClick={() => startEdit(f)} className="text-xs text-primary hover:underline"><Edit className="h-3 w-3 inline" /> Edit</button>
                  <button onClick={() => handleDelete(f.id)} className="text-xs text-red-500 hover:underline"><Trash2 className="h-3 w-3 inline" /> Delete</button>
                </div>
              </div>
            </motion.div>
          )
        })}
        {items.length === 0 && <div className="col-span-full text-center py-12 text-muted-foreground">No pricing formulas yet.</div>}
      </div>
    </div>
  )
}
