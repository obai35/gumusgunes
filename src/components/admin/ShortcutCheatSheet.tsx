  ...
  return (
    <>
      <button
        ...
        title={ta('Keyboard shortcuts (Ctrl+Shift+/)')}
      >
            ...
            >
              <div className="flex items-center justify-between px-5 py-4 border-b border-border">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-semibold text-foreground text-sm">{ta('Keyboard Shortcuts')}</h2>
                </div>
              ...
              <div className="p-5 space-y-1 max-h-80 overflow-y-auto">
                {DEFAULT_SHORTCUTS.map((s) => (
                  <div key={`${s.key}-${s.ctrl}`} className="flex items-center justify-between py-2">
                    <span className="text-sm text-foreground">{ta(s.description)}</span>
              ...
              <div className="px-5 py-3 bg-muted/30 border-t border-border">
                <p className="text-xs text-muted-foreground">{ta('Press ⌘/Ctrl + ⇧ + / to toggle this panel')}</p>
              </div>
            </motion.div>
          </motion.div>
        )}
