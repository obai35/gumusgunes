    ...
    <div className="bg-white rounded-xl border border-border p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-muted-foreground">
          {totalRevenue > 0 ? ta(`${fmtCurrency(totalRevenue)} total`) : ''}
        </span>
      </div>
      {products.length === 0 ? (
        <p className="text-sm text-muted-foreground py-4 text-center">{ta('No product data yet.')}</p>
      ) : (
        <div className="space-y-3">
          {products.map((p, i) => (
            <div key={p.id} className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-0.5">
                  ...
                  <span className="text-xs font-medium text-navy">{fmtCurrency(p.revenue)}</span>
                </div>
