...
export function FooterPanel() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="text-xs font-medium text-muted-foreground">{ta('Copyright Text')}</label>
        <input value={settings['footerCopyright'] || ''} onChange={e => updateSetting('footerCopyright', e.target.value)} className="w-full px-2 py-1.5 text-sm border border-border rounded mt-1" placeholder={ta('© Gümüş Güneş. All rights reserved.')} />
      </div>
      <div>
        <label className="text-xs font-medium text-muted-foreground">{ta('Email')}</label>
      ...
      <div>
        <label className="text-xs font-medium text-muted-foreground">{ta('Phone')}</label>
      ...
      <div>
        <label className="text-xs font-medium text-muted-foreground">{ta('Address')}</label>
      ...
      <div className="border-t border-border pt-3">
        <span className="text-xs font-medium text-muted-foreground block mb-2">{ta('Social Links')}</span>
      ...
      <div className="border-t border-border pt-3">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-medium text-muted-foreground">{ta('Link Columns')}</span>
          <button onClick={addCol} className="flex items-center gap-1 text-xs text-gold"><Plus className="h-3 w-3" /> {ta('Add Column')}</button>
        </div>
        {cols.map(col => (
          <div key={col._id} className="bg-gray-50 rounded-lg p-2 mb-2">
            <div className="flex items-center gap-1 mb-1">
              <input value={col.title} onChange={e => updateCol(col._id, e.target.value)} placeholder={ta('Column title')} className="flex-1 px-2 py-1 text-xs border border-border rounded font-medium" />
            ...
            {Array.isArray(col.links) && col.links.map(link => (
              <div key={link._id} className="flex items-center gap-1 ml-2 mb-1">
                <input value={link.label} onChange={e => updateLink(col._id, link._id, 'label', e.target.value)} placeholder={ta('Label')} className="flex-1 px-1.5 py-0.5 text-xs border border-border rounded" />
            ...
            <button onClick={() => addLink(col._id)} className="text-xs text-gold/70 hover:text-gold ml-2 mt-0.5">+ {ta('Add Link')}</button>
          </div>
        ))}
      </div>
    </div>
