...
const TIERS = [
  { name: 'Silver', min: 0, color: '#c0c5ce', perk: '5% off birthday gift' },
  { name: 'Gold', min: 500, color: '#d4af37', perk: '10% off + free shipping' },
  { name: 'Platinum', min: 2000, color: '#e5e4e2', perk: '15% off + early access' },
]
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span className="text-[10px] text-muted-foreground hidden sm:inline">pts</span>
          ...
          >
            ...
            <div className="navy-radial text-silver p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <div>
                  <p className="font-display text-sm font-semibold leading-tight">{current.name} Member</p>
                  <p className="text-[10px] text-silver/60">{points} points</p>
                </div>
              </div>
              {next && (
                <>
                  ...
                  <p className="text-[10px] text-silver/70">
                    {pointsToNext} pts to <strong className="text-gold">{next.name}</strong>
                  </p>
                </>
              )}
              {!next && (
                <p className="text-[10px] text-gold flex items-center gap-1">
                  <Sparkles className="h-3 w-3" /> Highest tier reached
                </p>
              )}
            </div>
            ...
            <div className="p-3 space-y-1.5">
              <p className="text-[10px] tracking-[0.2em] uppercase text-muted-foreground font-semibold px-1 mb-1">Membership Tiers</p>
              {TIERS.map((tier) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-muted-foreground line-clamp-1">{tier.perk}</p>
                    </div>
                    <span className="text-[10px] text-muted-foreground flex-shrink-0">{tier.min}+ pts</span>
                  </div>
                )
              })}
            </div>
            ...
            <div className="p-3 bg-secondary/30 border-t border-border">
              <p className="text-[11px] text-muted-foreground flex items-center gap-1.5">
                ...
                Earn 1 point per $1 spent. Points unlock exclusive perks.
              </p>
            </div>
          </motion.div>
        )}
